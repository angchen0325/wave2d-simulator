...
class SceneObject(ABC):
    ...
    def render(
        self,
        field: cupy.ndarray,
        wave_speed_field: cupy.ndarray,
        dampening_field: cupy.ndarray,
    ):
...
class WaveSimulator2D:
    ...
    def __init__(self, w, h, scene_objects, initial_field=None):
        ...
        self.c = cp.ones(
            (h, w), dtype=cp.float32
        )  # wave speed field (from refractive indices)
        ...
        if initial_field is not None:
            assert w == initial_field.shape[1] and h == initial_field.shape[2], (
                "width/height of initial field invalid"
            )
            self.u[:] = initial_field
            self.u_prev[:] = initial_field

        # Define Laplacian kernel
        self.laplacian_kernel = cp.array(
            [[0.066, 0.184, 0.066], [0.184, -1.0, 0.184], [0.066, 0.184, 0.066]]
        )
