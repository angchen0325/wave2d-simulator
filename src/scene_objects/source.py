...
class PointSource(SceneObject):
    ...
    def render(
        self,
        field: cp.ndarray,
        wave_speed_field: cp.ndarray,
        dampening_field: cp.ndarray,
    ):
        pass
...
class LineSource(SceneObject):
    ...
    def __init__(
        self, start, end, frequency, amplitude=1.0, phase=0, amp_modulator=None
    ):
        self.start = start
        self.end = end
        self.frequency = frequency
...
class ModulatorSmoothSquare:
    ...
    def __call__(self, t):
        ...
        a = (0.5 / math.atan(1.0 / s)) * math.atan(
            math.sin(t * self.frequency + self.phase) / s
        ) + 0.5
        return a
...
class ModulatorDiscreteSignal:
    ...
    def __call__(self, t):
        ...
        l = smooth_step(tf)
        interpolated_value = (1 - l) * self.signal_array[
            index_low
        ] + l * self.signal_array[index_high]
