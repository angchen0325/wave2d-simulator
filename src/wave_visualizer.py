...
class WaveVisualizer:
    ...
    def render_intensity(
        ...
        gray = (
            cp.clip((self.intensity**exp) * brightness_scale, 0.0, 1.0) * 254.0
        ).astype(np.uint8)
        img = (
    ...
    def render_field(self, brightness_scale=1.0, overlay_visualization=True):
        gray = (cp.clip(self.field * brightness_scale, -1.0, 1.0) * 127 + 127).astype(
            np.uint8
        )
        img = (
            self.field_colormap[gray].get()
...
def get_colormap_lut(name, invert, black_level=0.0, make_symmetric=False):
    ...
    elif name == "colormap_wave3":
        color_values = np.array(colormap_wave3) / 255
    elif name == "colormap_wave4":
        color_values = np.array(colormap_wave4) / 255
    else:
